...
window.addEventListener("click", function handler() {
  ...
  if (document.getElementById("musicprompt")) {
    document.getElementById("musicprompt").classList.add("fade-out");

    document.getElementById("musicprompt").addEventListener("transitionend", () => {
...
function closeReleaseCards(exceptCard = null) {
  document.querySelectorAll("[data-release-card]").forEach((card) => {
    ...
    const toggle = card.querySelector("[data-release-toggle]");
    const menu = card.querySelector("[data-release-menu]");
...
document.addEventListener("click", (event) => {
  if (!event.target.closest("[data-release-card]")) {
    closeReleaseCards();
  }
});
